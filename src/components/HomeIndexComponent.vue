    ...
    <section class="c">
      ...
      <div class="upload">
        ...
        <div class="upload-list" v-if="processFiles.length > 0">
          <n-flex class="summary" justify="space-between">
            <n-flex>
              <n-tooltip trigger="hover">
                <template #trigger>
                  <n-flex class="percent" :size="[0,0]">
                    <n-icon class="icon" color="green" size="38">
                      <CheckRound/>
                    </n-icon>
              ...
              <n-flex vertical justify="center" :size="[0,0]">
                <div>文件数<b>{{ uploadSummary.fileCount }}</b>，共省流：</div>
                <div>
          ...
          <n-flex class="item-container" :size="[0,12]">
            <n-flex class="item" v-for="uploadFile in processFiles" vertical :size="[0,10]">
...
import {isAndroid, isIOS, isMobile} from "vue-device-detect";
...
.c {
  ...
  .upload-list {
    ...
    .item:hover {
      background-color: rgba(81, 107, 118, .1);
    }
  }
